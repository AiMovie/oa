...
class Ueditor extends Controller {
	...
	function upload(){
		...
		$CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents(ROOT_PATH."/public/static/ueditor/config.json")), true);
		$this->config=$CONFIG;
		$action = $_GET['action'];
		switch ($action) {
			case 'config':
	...
	private function _ueditor_upload($config=array()){
		...
		if($file)
		{
			...
			if($info) {
				$state = 'SUCCESS';
				$url=substr(\think\Config::get('UPLOAD_DIR'),1).$info->getSaveName();//如果上传成功则完成路径拼接
			}else{
				$state = $file->getError();//否则就是上传错误，显示错误原因
			}			
		}
